...
template <size_t N> constexpr size_t str_len(const char (&)[N]) {
  return N - 1;
}
...
uint64_t get_header_value_u64(const Headers &headers,
...
uint64_t Request::get_header_value_u64(const std::string &key,
                                              uint64_t def, size_t id) const {
  return detail::get_header_value_u64(headers, key, def, id);
}

uint64_t Response::get_header_value_u64(const std::string &key,
                                               uint64_t def, size_t id) const {
  return detail::get_header_value_u64(headers, key, def, id);
}
...
uint64_t Result::get_request_header_value_u64(const std::string &key,
                                                     uint64_t def,
                                                     size_t id) const {
...
template <class Rep, class Period>
void ClientImpl::set_max_timeout(
    const std::chrono::duration<Rep, Period> &duration) {
  auto msec =
...
struct FileStat {
  explicit FileStat(const std::string &path);
  bool is_file() const;
  bool is_dir() const;
...
bool process_multipart_ranges_data(const Request &req,
  ...
  for (const auto & range : req.ranges) {
    ...
    auto offset_and_length =
        get_range_offset_and_length(range, content_length);
...
bool ClientImpl::redirect(Request &req, Response &res, Error &error) {
  ...
  if (next_scheme == scheme && next_host == host_ && next_port == port_) {
    ...
    if (next_scheme == "http" && next_host != host_) {
      // Forbid redirect to a different host when using HTTP
      return false;
    }
  }
...
void Client::set_max_timeout(time_t msec) {
